...
export async function GET(request: Request) {
  try {
    ...
    if (!canView) {
      return NextResponse.json({ error: 'Access denied. Finance view permission required.' }, { status: 403 });
    }
    ...
    const [
      ...
      prisma.expense.findMany({
        where: { orgId, expenseDate: { gte: startOfDay, lt: endOfDay }, approvalStatus: 'approved' },
      }),
      // Purchase orders
    ...
    const channels = await prisma.channelSource.findMany({ where: { id: { in: channelIds } }, select: { id: true, name: true } });
    ...
    const summary = {
      ...
      orders: ordersData.map((o: any) => ({ id: o.id, orderNumber: o.orderNumber, channel: o.channelSource?.name || 'Direct', amount: o.netAmount, tax: o.taxAmount, paymentStatus: o.paymentStatus, createdAt: o.createdAt })),
      expenses: expensesData.map((e: any) => ({ id: e.id, category: e.category, amount: e.amount, description: e.description, vendor: e.vendor })),
...
export async function POST(request: Request) {
  try {
    ...
    if (!canReconcile) {
      return NextResponse.json({ error: 'Access denied. Finance reconcile permission required.' }, { status: 403 });
    }
